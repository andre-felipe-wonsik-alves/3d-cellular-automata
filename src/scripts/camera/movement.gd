...
func get_input_direction() -> Vector3:
	...
	if Input.is_action_just_pressed("move_forward"): direction += Vector3.FORWARD;
	if Input.is_action_just_pressed("move_backward"): direction += Vector3.BACK;
	if Input.is_action_just_pressed("move_left"): direction += Vector3.LEFT;
	if Input.is_action_just_pressed("move_right"): direction += Vector3.RIGHT;
	if Input.is_action_just_pressed("move_up"): direction += Vector3.DOWN;
	if Input.is_action_just_pressed("move_down"): direction += Vector3.UP;
...
func _input(event: InputEvent) -> void:
	...
	if event is InputEventMouseMotion and Input.is_action_just_pressed("move_camera"):
